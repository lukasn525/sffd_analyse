...
% LITERATURDATENBANK
% ---------------------------------------------------
% Die Datenbank liegt seit dem 18.08.2026 als EIGENE DATEI literatur.bib.
% Sie ist zugleich das Quellenregister: Ueber jedem Eintrag stehen
% %%-Zeilen mit Status, Zugang und den FREIGEGEBENEN SEITEN. Biber
% ignoriert alles ausserhalb der @-Eintraege, die Angaben stoeren also nicht.
%
% ES WIRD NICHTS VON HAND IN literatur.bib GEAENDERT, was nicht vorher
% freigegeben ist. Geprueft wird das maschinell:
%
%     python tools/quellen.py
%
% Der Quellenwaechter bricht mit Exit-Code 1 ab, wenn
%   - eine Quelle zitiert wird, die nicht auf status=frei steht
%   - eine SEITE zitiert wird, die nicht in seiten= freigegeben ist
% Er erzeugt ausserdem docs/10_QUELLEN.txt und docs/quellen.html.
%
% Frueher stand die Datenbank hier als filecontents-Block. Das hatte zwei
% Nachteile: jede Quellenaenderung zwang zum Hochladen der kompletten
% main.tex, und die Registerdaten lagen in einer zweiten Datei -- genau die
% Doppelung, gegen die docs/03_STAND.md und tools/pruefe_zahlen.py sonst
% schuetzen.

% =========================================================================
% REGEL FUER DIESE DATENBANK -- ab 18.08.2026
% Es kommt NICHTS hier hinein, was nicht in docs/10_QUELLEN.md freigegeben
% ist. Die Datei ist das Register; diese Datenbank ist ihr Spiegel.
% Auch die SEITENZAHL ist freizugeben: zitiert werden darf nur eine Seite,
% die im Register unter "freigegebene Seiten" steht. Damit kann keine
% Seitenangabe mehr geraten werden.
% Am 18.08.2026 wurden sechs Eintraege wieder entfernt, die ohne Freigabe
% ergaenzt worden waren -- sie stehen jetzt im Register unter "in Pruefung".
% =========================================================================
% ---------------------------------------------------
...
%
% ============ OFFENE SEITENZAHLEN -- SIEBEN MAL "??" IN DIESEM ABSCHNITT ==
% Bewusst als ?? gesetzt statt geraten. Im PDF sichtbar, damit sie nicht
% untergehen. Nachzutragen sind GENAU ZWEI Werte:
%
%   a) Wirth & Hipp (2000), Abschnitt 3.2 "The Generic CRISP-DM Reference
%      Model" -- dort stehen alle sechs Phasendefinitionen auf derselben
%      Seite. Eine Zahl, sechsmal einzusetzen.
%   b) Schroeer et al. (2021) -- die Seite, auf der der Fortbestand von
%      CRISP-DM als Standard festgestellt wird.
%
% ACHTUNG, PAGINIERUNGSFALLE bei Wirth & Hipp: Der Eintrag in der Datenbank
% nennt die Tagungsband-Paginierung 29--39 (SCIRP fuehrt sogar 29--40, auch
% das ist zu klaeren). Die frei zugaenglichen Kopien sind dagegen ab 1
% paginiert. Eine Seitenangabe "3" waere in einem Beitrag, der auf Seite 29
% beginnt, unauffindbar. Entweder durchgehend nach Tagungsband zitieren --
% dann Vorspann addieren -- oder die Datenbank auf die tatsaechlich benutzte
% Fassung umstellen. NICHT mischen.
% Betrifft auch die bereits vorhandenen Angaben [1-2] und [526--532] bzw.
% [531--532] im Dokument: beide sind ungeprueft uebernommen.
% =========================================================================
...
Entwicklung von Data-Mining-Projekten.\footcite[??]{Schroeer2021}
% SEITENZAHL NACHTRAGEN. Wirth & Hipp (2000) kann "bis heute" nicht belegen --
% das Papier ist von 2000. Schroeer et al. (2021) ist die systematische
% Uebersicht, die den Fortbestand als Standard nachweist, und steht im Expose.
...
datengetriebenen Modellen im öffentlichen Sektor auf. Es stellt anschließend die
Referenzmodelle vor, an denen sich jedes weitere Verfahren messen lassen muss, und
darauf aufbauend die untersuchten Verfahren Ridge Regression, Random Forest und XGBoost.
Das Kapitel schließt mit der
...
werden.\footcite[??]{Wirth2000} Es beschreibt den Anwendungsfall und die fachliche Zielsetzung,
...
Kapitel 4 deckt die Phase \enquote{Data Understanding} ab.\footcite[??]{Wirth2000} Das Kapitel
...
denen aus den Rohdaten der finale Analysedatensatz konstruiert wird.\footcite[??]{Wirth2000} Neben
der Bereinigung und der Behandlung fehlender Werte wird insbesondere die zentrale
Entwurfsentscheidung zur räumlich-zeitlichen Aggregation begründet. Es folgen das
Feature-Engineering sowie der Analyserahmen mit der Aufteilung in Trainings- und Testmengen und
den Referenzwerten, an denen sich jedes spätere Modell messen lassen muss.
...
werden.\footcite[??]{Wirth2000} Es beschreibt den Versuchsaufbau einschließlich der
Validierungsstrategie und der technischen Umsetzung, begründet die Eignung der eingesetzten
Verfahren und dokumentiert die Modellkonfiguration sowie die Hyperparameter-Suchräume.
Abschließend werden die verfahrensspezifischen Konfigurationsentscheidungen und Tuning-Befunde
dargestellt.

Kapitel 7 ist der Phase \enquote{Evaluation} gewidmet.\footcite[??]{Wirth2000} Es stellt die
...
öffentlich verfügbaren Sekundärdaten nicht gegeben ist.\footcite[??]{Wirth2000} Die Auslassung
...
% Umfang: ca. 12 bis 16 Seiten -- siehe Seitenbudget in docs/09_SCHREIBPLAN.md
...
% penalisiertes lineares Modell, 2.5 wird dadurch kuerzer, nicht laenger.
%
% ABGRENZUNG NACH 5.4 -- die Regel aus dem Block vor Kapitel 4 gilt hier
...
\subsection{Grundlagen des überwachten Lernens}
% War 2.3.1. Jetzt eigenes Unterkapitel, weil es BEIDE nachfolgenden
% Modellklassen traegt -- die Referenzmodelle wie die Vergleichsverfahren --
% und deshalb in keine von beiden hineingehoert.
% Inhalt: Regression gegen Klassifikation, Verlustfunktion, Bias-Varianz-
% Zerlegung, Generalisierung. Kurz halten, eine halbe bis eine Seite.
% Beleg: Hastie et al. (2009), Kapitel 2 und 7.

\subsection{Referenzmodelle}
\label{sec:referenzmodelle}
% Rund anderthalb Seiten. Hier entstehen drei der Formeln, die das
% Formelverzeichnis bislang leer lassen.
%
% GLIEDERUNGSTIEFE -- bewusst flach (geaendert 18.08.2026):
% Zwei Referenzmodelle, zwei Unterabschnitte. Genauso wie 2.5 je einen
...
%
% ZWISCHEN 2.4 UND 2.4.1 STEHT KEIN TEXT -- Blindabsatz, Auflage 27.07.
...
% Auflage D) eroeffnet deshalb 2.4.1 und wird dort am konkreten Modell
...
% Die Kandidaten stehen in docs/10_QUELLEN.md unter "in Pruefung": Nelder &
...
% AUS DEM EXPOSE VERFUEGBAR und damit sofort einsetzbar: Hastie2009 und
% Cameron2013. Beide decken den Abschnitt weitgehend ab.
...
%
% DIE GRENZE DER MODELLKLASSE steht als SCHLUSSABSATZ am ENDE von 2.4.2.
...
% RUECKFALL, falls er sich beim Lesen verliert: als 2.4.3 mit eigener
...
% Dazu die Klassengewichtung als Mittel gegen unbalancierte Klassen -- hier
% nur das PRINZIP, die konkrete Wirkung auf Accuracy gegen Macro-F1 steht
% in 5.4 und 7.2.
% Belege: Agresti2013 (Kapitel 8, baseline-category logits), Hastie2009
%         (Kapitel 4.4).

% <- HIER der Schlussabsatz "Die Grenze der Modellklasse", ohne Ueberschrift.
%    Begruendung und Inhalt siehe Kopf von 2.4.

\subsection{Untersuchte Verfahren}
% ZWISCHEN 2.5 UND 2.5.1 STEHT KEIN TEXT (Blindabsatz). Die Einordnung --
% die Verfahren, die die Referenz aus 2.4 schlagen muessen, in der Ordnung
% linear, Bagging, Boosting -- eroeffnet 2.5.1.
% Keine Wiederholung der Grundlagen aus 2.3.

\subsubsection{Ridge Regression}
% Zielfunktion, L2-Regularisierung, Multikollinearität (Hoerl & Kennard 1970).
% Anknuepfen an 2.4: Ridge ist ein PENALISIERTES lineares Modell -- der
...
% ZWISCHEN 2.6 UND 2.6.1 STEHT KEIN TEXT (Blindabsatz).
...
%   Wahl des Poisson-GLM als Stufe-2-Baseline (#45)           -> 5  Festlegung
%   Baseline-WERTE (R2 0,542 / RMSE 33,98 usw.)               -> 5  Auflage
%                                                                  Schroeter:
%                                                                  Baseline in
%                                                                  die Data
%                                                                  Preparation
...
%   Effektive Stichprobe: 35 Stadtteile, davon 29 in der      -> 8  Limitation
%     Entwicklung; n_eff ~ 38 (ICC 0,926). Extrapolation
%
...
%      versaeumt.
%
% BLINDABSAETZE -- GILT FUER JEDE GLIEDERUNGSEBENE DER ARBEIT
%
% DIE REGEL, woertlich aus der Mitschrift vom 27.07.2026:
...
%
% Ein BLINDABSATZ ist also die BEWUSST LEERE Stelle zwischen einer
% Ueberschrift und ihrer ersten Unterueberschrift. "Blindabsaetze nutzen"
% heisst: diese Stelle LEER LASSEN. Also gilt:
%
...
%
% BEGRUENDUNG (Schroeter): Redundanz vermeiden. Ein Absatz an dieser Stelle
% kuendigt an, was gleich ohnehin kommt -- er ist ein vorweggenommenes
% Zwischenfazit. Der Inhalt gehoert in den ersten Unterabschnitt, wo er
% ohne Doppelung steht.
%
% ACHTUNG, DIESE STELLE WAR IN 01_VORGABEN.md FALSCH VERZEICHNET. Dort
% stand die Auflage vom 27.07. als "keine Blindabsaetze zwischen
% Gliederungsebenen" mit der Aufloesung "verbindende statt
% zusammenfassende Absaetze". Beides steht in keiner Mitschrift und ist am
% 18.08.2026 gegen die Primaerquellen korrigiert worden. Es gab nie einen
% Widerspruch zwischen dem 27.07. und dem 10.08.
%
% OFFENER VERSTOSS, Stand 18.08.2026: Kapitel 5 hat 13 Zeilen Fliesstext
% zwischen \section{Data Preparation} und \subsection{Datenauswahl und
% Bereinigung}. Der Text ist gut und inhaltlich noetig -- er muss nach 5.1
% wandern oder einen eigenen nummerierten Abschnitt bekommen.
% Alle uebrigen Kapitel sind regelkonform.
%
...
Im Data Understanding wurde zudem die Klassenverteilung der Einsatzart geprüft.

% ACHTUNG, EXPOSE-ANKUENDIGUNG IST UEBERHOLT. Der urspruengliche Satz endete mit
% "bei ausgepraegten Ungleichgewichten werden seltene Kategorien zusammengefasst
% oder die Klassifikation auf zwei Klassen vereinfacht, etwa Brand vs. Nicht-Brand".
% Das ist NICHT eingetreten: Die Zielgroesse hat vier Klassen (79,0 / 16,3 / 3,1 /
% 1,5 %), die Schieflage wird ueber class_weight="balanced" und Macro-F1 als
% Hauptmass behandelt, nicht ueber Zusammenlegung. Der Befund selbst gehoert
% hierher, die Behandlung nach 6.

% Hinweis: Hier steht nur der Befund (Klassenverteilung 79,0 / 16,3 / 3,1 / 1,5 %),
% die Konsequenz gehoert nach 5.3 (Bildung der vier Klassen per argmax) und die
% Behandlung der Schieflage nach 6 (class_weight, Macro-F1 statt Accuracy).
% ---- ENDE ÜBERNAHME ----
...
% ============================ BLINDABSATZ-VERSTOSS ======================
% Die folgenden zwei Absaetze stehen zwischen \section{Data Preparation} und
% \subsection{Datenauswahl und Bereinigung}. Nach der Auflage vom 27.07.
% ("Zwischen 1 und 1.1 steht kein Inhalt") gehoeren sie dort nicht hin.
% Inhaltlich sind sie richtig und noetig -- sie tragen die zentrale
% Entwurfsentscheidung (Analyseeinheit) und den Verfuegbarkeitsgrundsatz.
% ZWEI LOESUNGEN, beim Schreiben zu waehlen:
%   a) an den Anfang von 5.1 ziehen -- billig, aber 5.1 heisst "Datenauswahl
%      und Bereinigung" und die Analyseeinheit ist keine Bereinigung
...
%                            Dispersionsindex 62,8 (voller Datensatz) bzw.
%                            54,2 (Trainingsstadtteile Fold 1) -- BEIDE Werte
%                            sind korrekt, sie beziehen sich auf verschiedene
%                            Mengen; wo einer steht, ist die Bezugsmenge zu
%                            nennen (03_STAND.md Abschnitt 2).
%                            ACHTUNG: begruendet NICHT mehr die Negative
%                            Binomial. Seit #45 ist die Stufe-2-Baseline das
%                            unpenalisierte Poisson-GLM mit Offset. Die
%                            Ueberdispersion beschaedigt dort nur die
%                            Standardfehler, nicht die Konsistenz des
%                            bedingten Mittelwerts -- und eine Baseline mit
%                            reinen Punktvorhersagen verwendet keine
%                            Standardfehler. Begruendung gehoert nach 5.4.
...
%                            47,4 %. NUR Zwischengroesse: aus ihnen entsteht
%                            per argmax die dominante_einsatzart. Sie werden
%                            NICHT als eigene Zielgroessen modelliert oder
%                            berichtet -- entsprechend entfallen ihre
%                            Baselinewerte in 5.4.
...
%
% BERICHTET WIRD anzahl_einsaetze (#B10). einsaetze_je_1000_ew laeuft als
% Robustheitspruefung mit: ein Absatz plus Anhangstabelle, keine eigene
% Ergebnisreihe. Abbildungen zeigen weiterhin BEIDE Zielgroessen.
%
% Guetemasse RMSE / MAE / R2, je Zeile (Stadtteil x Monat). MASSGEBLICHE
% STREUUNG ist std_wiederholungen ueber die 10 Wiederholungsmittel, NICHT
% std_folds ueber die 50 Laeufe -- die 50 sind nicht unabhaengig, es sind
% dieselben 29 Stadtteile in anderer Gruppierung (R-5, Hurlbert 1984).
%
% REFERENZ ist die Stufe-2-Baseline aus 5.4, also das POISSON-GLM MIT OFFSET
% (R2 0,542 +/- 0,082, RMSE 33,98 +/- 3,11), nicht die Negative Binomial und
% nicht der Gesamtmittelwert. Der Gesamtmittelwert (Stufe 1, R2 -0,744) ist
% die Nullmarke und wird nur einmal genannt.
%
% ERGEBNIS, anzahl_einsaetze, Kreuzvalidierung (03_STAND.md 5.1):
%     Poisson-GLM    RMSE 33,98 +/- 3,11   MAE 22,94   R2 0,542
%     Random Forest       35,61 +/- 3,74       25,50      0,412
%     Ridge               36,51 +/- 2,77       24,39      0,514
%     XGBoost             37,77 +/- 3,26       25,81      0,510
%
% PRIMAERAUSSAGE (#34), gepaarter Wilcoxon auf den 10 Wiederholungsmitteln:
%     Random Forest gegen Baseline  -1,63  [-4,22; +0,96]   3/10  p 0,232
%                                          nicht unterscheidbar
%     Ridge   gegen Baseline        -2,53  [-4,27; -0,79]   2/10  p 0,010
%                                          signifikant schlechter
%     XGBoost gegen Baseline        -3,79  [-6,71; -0,88]   2/10  p 0,020
%                                          signifikant schlechter
%   KEIN VERFAHREN SCHLAEGT DIE BASELINE. Nur Random Forest erreicht ihr
%   Niveau, ohne es zu uebertreffen. Das ist die zentrale Aussage von 7.1 und
%   darf nicht relativiert werden.
%
% SEKUNDAER, Holm ueber 6 Tests: KEINE EINZIGE PAARUNG IST TRENNBAR
%   (p_holm 0,117 bis 0,967). R-1 ist vollstaendig eingetreten -- eine
%   Rangfolge zwischen den drei Verfahren ist nicht zulaessig. Berichtbar
%   bleibt ausschliesslich der Abstand zur Baseline.
%   FORMULIERUNGSAUFLAGE (B7): "Die Verfahren unterscheiden sich nicht" ist
%   NICHT GEZEIGT, sondern NICHT GEMESSEN. Effektstaerken d = 0,21 bis 0,87,
%   Trennschaerfe des gepaarten Wilcoxon bei n = 10 damit 10 bis 68 %. Der
%   Unterschied gehoert in den Text, sonst ist es die Ueberinterpretation
%   eines Nullbefunds.
%
% ROBUSTHEITSABSATZ zur Rate -- Richtung und Signifikanzmuster stimmen mit
%   anzahl_einsaetze ueberein, kein sekundaerer Vergleich wird signifikant
%   (Holm p >= 0,117). Begruendung fuer die Verdichtung: die Exposition steckt
%   seit #43 im Modell, und die R2-Werte der Rate streuen ueber die Folds
%   staerker als ihr Mittelwert (Random Forest 0,241 +/- 1,247), eine
%   Rangfolge waere dort nicht belastbar. Bei der Rate RMSE/MAE berichten,
%   R2 nur nachrichtlich (Begruendung in 5.4).
%                                                          -> UF2
...
% Drei Punkte gehoeren zwingend hierher:
%
% - DIE ERWARTUNG AUS 2.7 IST WIDERLEGT. Dort steht, methodisch sei zu
%   erwarten, dass die baumbasierten Verfahren am staerksten abschneiden
%   (Grinsztajn et al. 2022). Gemessen schlaegt im Mengenstrang KEIN
%   Verfahren die Stufe-2-Baseline, und im Strukturstrang kehrt sich das
%   CV-Ergebnis auf dem Hold-out um. Das ist der interessanteste Befund der
%   Arbeit und gehoert an den Anfang von 8.1, nicht ans Ende.
%
% - SPEZIFIKATION SCHLAEGT VERFAHRENSWAHL -- die Antwort auf UF4.
%   ACHTUNG, DIE FRUEHERE FASSUNG DIESES BLOCKS IST UEBERHOLT. Sie sprach von
%   einer "Spezifikationsasymmetrie", weil nur die Baseline log(Bevoelkerung)
%   als Offset bekomme. Seit #43 modellieren ALLE Verfahren die Rate und
%   multiplizieren mit der Einwohnerzahl zurueck; die Asymmetrie ist
%   konstruktiv beseitigt, nicht nur benannt. Verblieben ist ein SCHWAECHERER
%   Rest: Das GLM traegt log(Bevoelkerung) als Offset mit Koeffizient 1, die
%   uebrigen Verfahren muessen die Groessenbeziehung ueber die
%   Ruecktransformation abbilden. Das ist zu benennen, aber es ist kein
%   ungleicher Vergleich mehr.
%   DIE ZAHLEN, die daraus das Hauptargument machen (03_STAND.md 5.5):
%     zwischen den drei VERFAHREN liegen           2,2 RMSE
%     zwischen den SPEZIFIKATIONEN eines Verfahrens 19 bis 29 RMSE
%     Random Forest ohne Expositionsbehandlung   64,81 statt 35,61
%     XGBoost      ohne Expositionsbehandlung    56,90 statt 37,77
%   Ueber die Spezifikationsgegenprobe hinaus: Verfahrenswahl bewegt bis 2,5
%   RMSE, Spezifikationswahl bis 146,9 (Abbildung A3).
%   Der Grund ist die multiplikative Groessenstruktur: Baumverfahren geben je
%   Blatt einen festen Wert aus und koennen "Einsaetze = Bevoelkerung x
%   Risiko" nicht abbilden; Verfahren mit Log-Verknuepfung bekommen die
%   Multiplikation geschenkt.
%
% - UNTERSCHEIDBARKEIT. Bei 29 Entwicklungsstadtteilen streuen die
%   Fold-Ergebnisse stark; keine einzige Verfahrenspaarung ist trennbar
%   (R-1 vollstaendig eingetreten). Wo sich die Streuungsbereiche
%   ueberlappen, ist das so zu berichten und nicht als Rangfolge zu
%   kaschieren (Gutachten R6). Zugleich die Formulierungsauflage aus B7
%   beachten: nicht gezeigt, sondern nicht gemessen.
%
% - UEBERANPASSUNG als eigenes Argument (B6, gehoert teils nach 7.3):
%   R2 Training gegen R2 Kreuzvalidierung bei anzahl_einsaetze --
%   ridge 0,826/0,514 (Differenz 0,313), xgboost 0,983/0,510 (0,473),
%   random_forest 0,984/0,412 (0,572). Random Forest braucht 861-mal laenger
%   als Ridge und generalisiert dabei am schlechtesten. Das ist ein sauberes
%   Effizienzargument und die belastbarste Aussage der Arbeit, weil hier
%   Groessenordnungen liegen und nicht Prozentpunkte.
...
% Aufzunehmen: 35 Stadtteile, davon 29 in der Entwicklung -- die effektive
% Stichprobe liegt bei rund 38 Einheiten bei 4.620 Zeilen (ICC 0,926,
% Designeffekt 122); Extrapolation bei unbekannten Stadtteilen (33,7 % der
% Testzeilen in Wiederholung 0, 34,6 % ueber alle 50 Laeufe, 7,6 % im
% Hold-out -- die Bezugsmenge ist jeweils zu nennen), und zwar als
% Eigenschaft von STADTTEILEN: 9 von 29 liegen mit 100 % ihrer Zeilen
% ausserhalb, 16 von 29 mit 0 %; Hold-out aus nur 6 Stadtteilen, einmalig
% und ohne Streuung ausgewertet, deshalb dort KEINE Rangfolge (R-4);
% jaehrliche ACS-Merkmale und Land-Use-Snapshot 2020 (die drei baulichen
% Merkmale sind je Stadtteil KONSTANT -- 1 eindeutiger Wert ueber 132 Monate,
% gegen 128,6 beim Kriminalitaetsindex); ACS-Trefferquote 2009 von 63,1 %
% gegen 79,7 % (2014/2019) und 99,2 % (2021/2023); sowie die Abweichungen
...
%
% DER HOLD-OUT IST LEICHTER ALS DIE KREUZVALIDIERUNG -- BENENNEN (B8).
% Extrapolationsanteil 7,6 % gegen 34,6 %, und trainiert wird auf 29 statt 23
% Stadtteilen. Alle Hold-out-Werte liegen deshalb ueber den CV-Werten. Ohne
% diesen Hinweis wirkt die Schlussbewertung geschoent. Die beiden Zahlenreihen
% duerfen NICHT direkt verglichen werden.
...
%
% ===================================================================
% DIE EFFEKTIVE STICHPROBE -- WIE SIE ZU SCHREIBEN IST (B-49, B-50)
% ===================================================================
% Die Zahl lautet: rund 38 effektive Einheiten bei 4.620 Zeilen (ICC 0,926,
% Designeffekt 122). Ohne Saison und Kriminalitaetsindex nehmen die Merkmale
% nur 140 verschiedene Werte an.
%
...
%                      Ein Seitenhieb auf die Literatur OHNE BELEG. In einer
%                      Bachelorarbeit ist das angreifbar, nicht souveraen.
%                      NICHT SO SCHREIBEN.
%
%   RICHTIG, sachlich: Die Groessenordnung als Eigenschaft des Gegenstands
%                      benennen, die eigene Behandlung dagegenstellen, beides
%                      belegen. Etwa:
%
...
%      Einzellaeufen, weil letztere Pseudoreplikation waeren (Hurlbert 1984);
%      validiert wird stadtteilweise statt zufaellig, weil zufaelliges k-Fold
%      bei Abhaengigkeitsstrukturen zu optimistische Schaetzungen liefert
%      (Roberts et al. 2017). Die berichtete Streuung quantifiziert die
%      Abhaengigkeit von der Fold-Zuteilung und ist damit eine UNTERGRENZE der
%      Unsicherheit ueber die Grundgesamtheit der Stadtteile."
%
% BELEGE, die diese Fassung tragen -- beide sind kanonisch und zitierfaehig:
%   Hurlbert, S. H. (1984): Pseudoreplication and the Design of Ecological
%     Field Experiments. Ecological Monographs 54(2), S. 187-211.
%     -> die Referenz fuer Pseudoreplikation ueberhaupt
%   Roberts, D. R. et al. (2017): Cross-validation strategies for data with
%     temporal, spatial, hierarchical, or phylogenetic structure.
%     Ecography 40(8), S. 913-929.
%     -> Block-/Leave-Group-Out-CV als empfohlene Praxis; ENTLASTUNG, weil der
%        Stadtteil-Split damit Standard ist und kein Sonderweg
%
% WARUM DAS IN DIE ARBEIT GEHOERT UND NICHT NUR INS KOLLOQUIUM: Wer die Zahl
% selbst nennt und methodisch beantwortet, nimmt der Frage die Spitze. Wer sie
% verschweigt, laesst sie sich stellen. Das Gutachten bewertet nach R6/R9
% nicht, ob Probleme auftreten, sondern ob sie methodisch geloest wurden.
%
% -------------------------------------------------------------------
% DIE FORMULIERUNG OBEN IST EIN GERUEST, KEINE VORLAGE.
% Eigener Satzbau, eigene Reihenfolge, eigene Laenge - und Schroeters
% Stilvorgaben (keine Kursivschrift, keine Anfuehrungszeichen, keine
% Zwischenfazits, Blindabsaetze) gelten ohnehin. FREI sind Ton, Aufbau und
% Umfang. ERHALTEN BLEIBEN MUESSEN nur die pruefbaren Behauptungen:
%
%   - die Zahlen: n_eff ~ 38, ICC 0,926, Designeffekt 122, 140 Merkmalsvektoren
%   - Hurlbert (1984) an der Stelle "warum n = 10 und nicht 50"
%   - Roberts et al. (2017) an der Stelle "warum stadtteilweise, nicht zufaellig"
...
% "das Konfidenzintervall ist konservativ" oder "vorsichtig geschaetzt".
% Das ist das GEGENTEIL und waere falsch. Konservativ hiesse zu breit; hier
% ist es zu SCHMAL, weil alle zehn Wiederholungen dieselben 29 Stadtteile
% teilen (B-50). Sichere Wendungen: "eher zu schmal als zu breit", "die
% tatsaechliche Unsicherheit liegt darueber".
%
% KI-VERZEICHNIS: Wird eine hier vorformulierte Passage als Geruest genutzt,
% gehoert ein Eintrag in die Tabelle am Ende des Dokuments - nach dem Muster
% der vorhandenen Eintraege mit Prompt und Ergebnisverweis.
% ===================================================================
...
% A1  Vollstaendige Attributtabelle -- Quelle results/codebook/merkmale.md,
%     erzeugt von tools/codebook.py (Auflage Schroeter 10.08.: eine grosse
%     Tabelle mit Skalenniveau, Was/Wie/Wofuer). 34 Spalten.
% A2  Code-Auszuege. ACHTUNG ZUM SEITENBUDGET: Codeausschnitte ZAEHLEN zu den
%     40-60 Inhaltsseiten, Abbildungen NICHT (Schroeter 10.08.). Die rund 20
%     geforderten Ausschnitte sind damit der teuerste Posten der Arbeit --
%     ausdruecklich verlangt sind Serialisierung und Multithreading.
% A3  Ergaenzende Abbildungen. Zehn liegen erzeugt vor in
%     results/abbildungen/ (a0_pipeline, a1_gegen_baseline, a2_foldstruktur,
%     a3_spezifikation, a4_laufzeit_guete, a5_holdout, a6_faktorgruppen,
%     a7_extrapolation, a8_hyperparameter, a9_parallelisierung,
%     a10_qq_residuen). Alle als PDF, also vektoriell -- Auflage
%     "Vektorformat" erfuellt. Sie sind BISLANG NICHT EINGEBUNDEN: main.tex
%     enthaelt genau ein \includegraphics, das FOM-Logo.
% A4  Anhangstabelle einsaetze_je_1000_ew (Robustheitspruefung, #B10).
% A5  Ergebnisse der 15 bzw. 9 Signifikanztests im Ueberblick.
