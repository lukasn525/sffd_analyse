...
Antwortzeit (Code~\ref{lst:gesperrt}). Sie sind nicht nur unzulässig,
...
\label{lst:gesperrt}\vspace{-3.5pt}
\begin{lstlisting}[style=pythonstil,firstnumber=182]
ERGEBNISVARIABLEN = [
...
\end{lstlisting}
\tabquelle[Eigene Darstellung, \texttt{prep/config.py}, Z.~182--187]
\end{quellcode}
...
(Code~\ref{lst:quoten}). Ein Nenner ist begründungspflichtig: Der
...
\begin{lstlisting}[style=pythonstil,firstnumber=601]
QUOTEN = [
    ...
    ("pct_pre1940",      "pre1940_count",         "yrbuilt_count"),(*@\setcounter{lstnumber}{606}@*)
    ("pct_residential",  "residential_count",     "parcel_count"),(*@\setcounter{lstnumber}{609}@*)
](*@\setcounter{lstnumber}{612}@*)
def berechne_quoten(df: pd.DataFrame) -> pd.DataFrame:(*@\setcounter{lstnumber}{621}@*)
    ...
    return df
\end{lstlisting}
\tabquelle[Eigene Darstellung, \texttt{prep/s1\_daten.py}, Z.~601--629, gekürzt]
\end{quellcode}
